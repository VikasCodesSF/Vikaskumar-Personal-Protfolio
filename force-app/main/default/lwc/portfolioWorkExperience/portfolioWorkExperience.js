...
export default class PortfolioWorkExperience extends LightningElement {
    ...
    formatExperience(data){
        this.workExperienceList = [...data.records].reverse().map(item=>{
            let id = item.id
            const {JobStartDate__c,JobEndDate__c, CompanyName__c, Role__c, 
                Description__c, WorkLocation__c, isCurrent__c} = item.fields
            ...
            return {id, JobStartDate, JobEndDate, CompanyName, Role, Description, WorkLocation,  isCurrent}
